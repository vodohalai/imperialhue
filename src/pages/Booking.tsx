...
const Booking = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setDone(true);
    showSuccess("Đặt phòng thành công! Lễ tân sẽ liên hệ xác nhận.");
  };
        ...
        <div className="mx-auto max-w-5xl">
          ...
          {done ? (
            <div className="rounded-[2rem] bg-white p-8 text-center shadow-sm">
              ...
              <h1 className="mt-4 text-2xl font-black text-slate-900">Cảm ơn bạn, {name || "quý khách"}!</h1>
              <p className="mt-2 text-sm text-slate-600">
                Yêu cầu đặt {room.name} từ {checkIn} đến {checkOut} đã được tiếp nhận. Đội ngũ lễ tân sẽ liên hệ trong vòng 1 giờ.
              </p>
            ...
            <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
              <form onSubmit={handleSubmit} className="rounded-[2rem] border border-[#ece6dd] bg-white p-6 shadow-sm">
                <p className="text-sm font-semibold uppercase tracking-[0.25em] text-[#f97316]">Đặt phòng</p>
                <h1 className="mt-2 text-2xl font-black text-slate-900">Hoàn tất thông tin đặt phòng</h1>
                ...
                <div className="mt-6 grid gap-4 md:grid-cols-2">
                  <input required value={name} onChange={(e) => setName(e.target.value)} className="rounded-2xl border border-[#e7e0d6] bg-[#fbfaf7] px-4 py-3 text-sm outline-none" placeholder="Họ và tên" />
                  <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="rounded-2xl border border-[#e7e0d6] bg-[#fbfaf7] px-4 py-3 text-sm outline-none" placeholder="Email" />
                  <input required value={phone} onChange={(e) => setPhone(e.target.value)} className="rounded-2xl border border-[#e7e0d6] bg-[#fbfaf7] px-4 py-3 text-sm outline-none md:col-span-2" placeholder="Số điện thoại" />
                  <textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="min-h-28 rounded-2xl border border-[#e7e0d6] bg-[#fbfaf7] px-4 py-3 text-sm outline-none md:col-span-2" placeholder="Yêu cầu thêm (nếu có)" />
                </div>
              ...
              <aside className="rounded-[2rem] border border-[#ece6dd] bg-white p-6 shadow-sm">
                <img src={room.images[0]} alt={room.name} className="h-44 w-full rounded-2xl object-cover" />
                <h2 className="mt-4 text-xl font-bold text-slate-900">{room.name}</h2>
                <p className="text-sm text-slate-500">{room.size} · {room.bed}</p>
                ...
                <div className="mt-4 space-y-3 text-sm text-slate-600">
                  ...
                  <div className="flex items-center justify-between">
                    <span>Giá / đêm</span>
                    <span className="font-semibold text-slate-900">{formatPrice(room.price)}</span>
                  </div>
                </div>
